...
function parseData(data){
  ...
  for (var i = 0, len = data.length; i < len; ++i) {
    ...
    allAges.push(data[i]["All Ages"]);
    child.push(data[i])["Age 0-20"];
    youngAdult.push(data[i]["Age 21-34"]);
    adult.push(data[i]["Age 35-54"]);
    older.push(data[i]["Age 55+"]);
...
function createCharts() {
  ...
  var map = Highcharts.chart('map', {
    ...
    title: {
        text: 'Motor Vehicle Death Rates in all 50 States'
    },
    ...
    tooltip: {
        headerFormat: '',
        pointFormat: 'The motor vehicle death rate in <b> {point.name}</b> is <b>{point.value}</b>'
    },
